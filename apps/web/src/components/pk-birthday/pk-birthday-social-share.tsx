...
import type { PkBirthdayCountdownPayload } from "@/lib/api-client";

type Props = {
  share: PkBirthdayCountdownPayload["share"];
  size?: "sm" | "default";
  className?: string;
};

export function PkBirthdaySocialShare({ share, size = "default", className = "" }: Props) {
  async function copyShare() {
    try {
      await navigator.clipboard.writeText(share.copyText);
      toast.success("Copied — paste on Instagram, Facebook, or anywhere!");
    } catch {
